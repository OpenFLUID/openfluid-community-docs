...
factorNames=c("cellSU","cellRS","sigma",paste("thi",(1:6),sep='_'),paste("Ks",(1:6),sep='_'))
nFactor=length(factorNames)
factorVal=rep(NA,n=nFactor)
...
#
getSpatialResults <- function(ofsim,unitclass,unitids,OFnames,shortnames,out=NULL) {
...
aggregateSelectedSimResults <- function(ofsim,outProvided=NULL) {
    ...
    SU5RR = sum(out$SU5RR) * as.numeric(OpenFLUID.getAttribute(ofsim,"SU",5,"area"))
    SU5I  = sum(out$SU5I) * as.numeric(OpenFLUID.getAttribute(ofsim,"SU",5,"area"))
    SU5V  = sum(out$SU5Q) * dT
    # time for maximum water discharge at the SU#5 outlet (spatialized output)
    SU5TMax = as.numeric(
        difftime(out$datetime[which.max(out$SU5Q)],out$datetime[which.max(out$SU5RR)],units='hour')
    )
    if (SU5TMax < 0) SU5TMax=NA
    # cumulative rain and infiltration in the whole catchment (global outputs)
    SURR = 0
    SUI  = 0
    for (SUID in OpenFLUID.getUnitsIDs(ofsim,"SU")) {
        SUArea = as.numeric(OpenFLUID.getAttribute(ofsim,"SU",SUID,"area"))
...
cat("\n - mass balance error            :",aggInd$sumSU5RR-(aggInd$sumSU5I+aggInd$sumSU5V))
...
plotSimResults <- function(ofsim,outProvided=NULL) {
    ...
    for (SUID in OpenFLUID.getUnitsIDs(ofsim,"SU")) {
        SUArea = as.numeric(OpenFLUID.getAttribute(ofsim,"SU",SUID,"area"))
        SURR   = SURR + out[,paste("SU",SUID,"RR",sep='')]*SUArea
        SUI    = SUI  + out[,paste("SU",SUID,"I",sep='')]*SUArea
    }
    out = cbind(out,"SURR"=SURR)
    out = cbind(out,"SUI"=SUI)
    # water volume exported at the subcatchment outlet
    out = cbind(out,"RS3V"=out$RS3Q*dT)
    ...
    gpSU5 = ggplot(out,aes(x=datetime)) + theme_bw() +
        geom_bar(aes(y=SU5RR,color="rain"),fill="lightblue",stat="identity",alpha=.5,size=0.25) +
        geom_line(aes(y=SU5I,color="infiltration"),size=1) +
        geom_line(aes(y=SU5H,color="exported"),size=1) +
        labs(title="SU5",x="time",y="height [m]",color="") +
    ...
    gpRS3 = ggplot(out,aes(x=datetime)) + theme_bw() + 
        geom_bar(aes(y=SURR,color="rain"),fill="lightblue",stat="identity",alpha=.5,size=0.25) +
        geom_line(aes(y=SUI,color="infiltration"),size=1) +
        geom_line(aes(y=RS3V,color="exported"),size=1) +
        labs(title="subcatchment (outlet on RS3)",x="time",y="volume [m3]",color="") +
        scale_color_manual(values=colors) + 
        theme(legend.title=element_blank()) + theme(legend.position="bottom")        
...
plotDOECoverage <- function(DOE) {
    ...
    return(
        ggplot(DOECov, aes(x=name,y=relval) ) +
            ...
            geom_bin2d(bins=c(5*nFactor,100)) + theme_bw() + scale_fill_continuous(type = "viridis")
    )
}
...
runMultiSim <- function(X,ofsim) {
    ...
    for (i in rownames(X)) {
        ...
        cat("\rsimulations in progress ",rep("o",nInProgress),rep("-",20-nInProgress)," [",i,"/",NSimu,"]",sep='')
        # run current simulation
        runOneSim(as.numeric(X[i,]),ofsim)
        # get and store results
        out = getSelectedSimResults(ofsim)
        outByTimeStep[[paste("simu",i,sep='')]] = out
        outIndicators = rbind(outIndicators,aggregateSelectedSimResults(ofsim,out))
    }
    cat("\nall ",NSimu,"simulations done.\n")
    return(list("indicators"=outIndicators,"byTimeStep"=outByTimeStep))
...
boxSize=c(0.1,0.1)
boundingBox=data.frame("x"=(boxCenter[1]-0.5*boxSize[1]),"y"=(boxCenter[2]-0.5*boxSize[2]))
boundingBox=rbind(boundingBox,data.frame("x"=(boxCenter[1]-0.5*boxSize[1]),"y"=(boxCenter[2]+0.5*boxSize[2])))
boundingBox=rbind(boundingBox,data.frame("x"=(boxCenter[1]+0.5*boxSize[1]),"y"=(boxCenter[2]+0.5*boxSize[2])))
boundingBox=rbind(boundingBox,data.frame("x"=(boxCenter[1]+0.5*boxSize[1]),"y"=(boxCenter[2]-0.5*boxSize[2])))
boundingBox=rbind(boundingBox,data.frame("x"=(boxCenter[1]-0.5*boxSize[1]),"y"=(boxCenter[2]-0.5*boxSize[2])))
...
iMatchBox = intersect(
    which( (Y$indicators$tmaxRS3Q>boundingBox[1,1]) & (Y$indicators$tmaxRS3Q<boundingBox[3,1]) ),
    which( (Y$indicators$sumRS3V>boundingBox[1,2]) & (Y$indicators$sumRS3V<boundingBox[3,2]) )
)
...
boundingBoxSU5=rbind(boundingBoxSU5,data.frame("x"=(boxCenterSU5[1]-0.5*boxSize[1]),"y"=(boxCenterSU5[2]+0.5*boxSize[2])))
boundingBoxSU5=rbind(boundingBoxSU5,data.frame("x"=(boxCenterSU5[1]+0.5*boxSize[1]),"y"=(boxCenterSU5[2]+0.5*boxSize[2])))
boundingBoxSU5=rbind(boundingBoxSU5,data.frame("x"=(boxCenterSU5[1]+0.5*boxSize[1]),"y"=(boxCenterSU5[2]-0.5*boxSize[2])))
boundingBoxSU5=rbind(boundingBoxSU5,data.frame("x"=(boxCenterSU5[1]-0.5*boxSize[1]),"y"=(boxCenterSU5[2]-0.5*boxSize[2])))
...
gpVolTMaxWithBox$SU5 + 
    geom_line(data=boundingBoxSU5[1:4,],aes(x=x,y=y,color="red"),size=1.5) +
    geom_line(data=boundingBoxSU5[4:5,],aes(x=x,y=y,color="red"),size=1.5)
dev.off()

# drawing factors domain coverage
iMatchBoxSU5 = iMatchBox[intersect(
    which( (Y$indicators$tmaxSU5Q[iMatchBox]>boundingBoxSU5[1,1]) & (Y$indicators$tmaxSU5Q[iMatchBox]<boundingBoxSU5[3,1]) ),
    which( (Y$indicators$sumSU5V[iMatchBox]>boundingBoxSU5[1,2]) & (Y$indicators$sumSU5V[iMatchBox]<boundingBoxSU5[3,2]) )
)]
...
for (i in 1:length(iMatchBoxSU5)) {
    name = iEffect-0.9
    relval = as.numeric((X$X[iMatchBoxSU5[i],]-factorMin)/(factorMax-factorMin))
    color  = rainbow(length(iMatchBoxSU5))[i]
    gpFactors = gpFactors + 
        geom_line(data=data.frame(name,relval,color),aes(x=name,y=relval,color=color),size=0.5,linetype="dashed")        
}
gpFactors
dev.off()
```
